...
| **L3** | `UiHarvest` (`Widgets.Label`, `ButtonText`, `TooltipHandler`, `CheckboxLabeled`, `FloatMenuOption`) | Хардкод в C# чужих модов, кнопки, подсказки и меню | Безопасный асинхронный перехват UI на лету (`context=ui`) |
| **Dynamic** | `DynamicTranslator` (`QuestManager.Add`, `LetterStack.ReceiveLetter`, `MainTabWindow_Quests.PreOpen`, `Messages.Message`, `DiaNode`) | Задания, описания квестов, письма, диалоги и ивенты в реальном времени | Перевод на лету с сохранением заголовков и шаблонов |

Слой **L3 (UiHarvest V2)** безопасно выгружает новые видимые элементы UI в фоновые потоки без аллокаций во время OnGUI, а в кадрах выполняет быстрый $O(1)$ поиск готовых переводов из памяти.
...
- `ui` — перехваченные на лету кнопки, метки и подсказки сторонних модов.
